...
public class ColorTable extends GameObject implements DaltonicListener {
   ...
   ArrayList<ColoringButton> coloringButtons;
    ...
    public void init() {
        for(int i= 0; i<this.numColors;i++){
            IImage img= getEngine().getGraphics().newImage("images/frog/basic-"+ i +".png");
            this.coloringButtons.add( createButton(i,img, getX() +25*(i+1)));
        }
        super.init();
    }

    private ColoringButton createButton(int index, IImage img, int posX)
    {
        ColoringButton coloringButton= new ColoringButton(getScene(), font,index, img);
